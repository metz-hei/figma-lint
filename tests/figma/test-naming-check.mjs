...
// Имя варианта внутри сета генерирует Figma из значений свойств, переименовать
// его нельзя — проверяем только props, иначе каждый вариант даёт ложную ошибку.
...
assert(
  "имя варианта внутри сета не проверяется",
...
assert(
  "props варианта внутри сета проверяются по-прежнему",
